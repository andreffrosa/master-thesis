...
static bool AHBPPolicyEval(ModuleState* policy_state, PendingMessage* p_msg, unsigned char* myID, RetransmissionContext* r_context, list* visited) {
    ...
    bool retransmit = RP_eval(delegated_neighbors_policy, p_msg, myID, r_context, visited);
    ...
    if( mobility_extension && !retransmit ) {
        ...
        if(!RC_query(r_context, "neighborhood", &neighborhood, NULL, myID, visited2))
            assert(false);
...
static void AHBPPolicyDestroy(ModuleState* policy_state, list* visited) {
    RetransmissionPolicy* delegated_neighbors_policy = ((RetransmissionPolicy*)policy_state->vars);

...
void DF_init(discovery_framework_state* state) {
    ...
    state->neighbor_change_send_hello = false;
    state->neighbor_change_send_hack = false;
...
void deliverHello(void* f_state, HelloMessage* hello, WLANAddr* addr) {
    DF_uponHelloMessage((discovery_framework_state*)f_state, hello, addr);
}

void deliverHack(void* f_state, HackMessage* hack) {
    DF_uponHackMessage((discovery_framework_state*)f_state, hack);
}
...
void DF_sendMessage(discovery_framework_state* state, HelloMessage* hello, HackMessage* hacks, byte n_hacks, WLANAddr* addr) {
    ...
    DA_createDiscoveryMessage(state->args->algorithm, state->myID, &state->current_time, state->neighbors, false, hello, hacks, n_hacks, buffer, &buffer_size);
...
void DF_uponHelloTimer(discovery_framework_state* state) {
    ...
    if( DA_periodicHello(state->args->algorithm) ) {
        ...
        if( compare_timespec(&state->current_time, &state->next_hello_time) >= 0 ) {
            ...
            bool piggyback_hack = hack_piggyback_type == PIGGYBACK_ON_BROADCAST_TRAFFIC || hack_piggyback_type == PIGGYBACK_ON_DISCOVERY_TRAFFIC || hack_piggyback_type == PIGGYBACK_ON_ALL_TRAFFIC;
            if( piggyback_hack ) {
            ...
            DF_sendMessage(state, &hello, hacks, n_hacks, NULL);
    ...
    if( DA_periodicHack(state->args->algorithm) ) {
        ...
        if( compare_timespec(&state->current_time, &state->next_hack_time) >= 0 ) {
            ...
            if( piggyback_hello ) {
                ...
                DF_sendMessage(state, NULL, hacks, n_hacks, NULL);
            }
...
void DF_uponReplyTimer(discovery_framework_state* state, unsigned char* timer_payload, unsigned short timer_payload_size) {
    ...
    if( piggyback_hello ) {
        ...
        DF_sendMessage(state, &hello, &hack, n_hacks, addr);
    } else {
        DF_sendMessage(state, NULL, &hack, n_hacks, addr);
    }
...
bool DF_uponNeighborTimer(discovery_framework_state* state, NeighborEntry* neigh) {
    ...
    if( compare_timespec(rx_exp_time, &state->current_time) > 0 ) {
        ...
        bool lost_bi = false, updated_quality = false;
        ...
        if( missed_hellos > prev_missed_hellos ) {
            unsigned int lost_messages = 1;

            state->stats.missed_hellos++;

            // Update Link Quality
            double old_rx_lq = NE_getRxLinkQuality(neigh);
            double new_rx_lq = DA_computeLinkQuality(state->args->algorithm, NE_getLinkQualityAttributes(neigh), old_rx_lq, 0, lost_messages, false, &state->current_time);
            NE_setRxLinkQuality(neigh, new_rx_lq);

            // Check if the new quality changed too much
            if( fabs(old_rx_lq - new_rx_lq) >= state->args->lq_threshold ) {
                updated_quality = true;

                // printf("updated rx lq due to timer\n");
        ...
        hash_table* ht = NE_getNeighborNeighs(neigh);
        void* iterator = NULL;
        hash_table_item* hit = NULL;
        while( (hit = hash_table_iterator_next(ht, &iterator)) ) {
            NeighTuple* nn = (NeighTuple*)hit->value;
        ...
        if( lost_bi || updated_quality || deleted > 0 ) {
        ...
        if( !NE_isDeleted(neigh) ) {
            ...
            scheduleNeighborChange(state, false, true, false, false);
...
void DF_uponNeighborChangeTimer(discovery_framework_state* state) {
    ...
    if( state->neighbor_change_send_hello ) {
        if( compare_timespec(&state->set_neighbor_change_time, &state->last_hello_time) > 0 ) {
            ...
            DF_uponHelloTimer(state);
        } else {
            // Do nothing
        }
    }

    if( state->neighbor_change_send_hack ) {
    ...
    #ifdef DEBUG_DISCOVERY
    char str[200];
    sprintf(str, "HELLO=[%s] HACK=[%s]", (state->neighbor_change_send_hello?"yes":"no"), (state->neighbor_change_send_hack?"yes":"no"));
...
void scheduleNeighborChange(discovery_framework_state* state, bool new_neighbor, bool lost_neighbor, bool neighbor_update, bool other) {
    ...
    bool send_hello = other || (DA_HelloNewNeighbor(alg) && new_neighbor) || (DA_HelloLostNeighbor(alg)  && lost_neighbor) || (DA_HelloUpdateNeighbor(alg) && neighbor_update);

    bool send_hack = other || (DA_HackNewNeighbor(alg) && new_neighbor) || (DA_HackLostNeighbor(alg)  && lost_neighbor) || (DA_HackUpdateNeighbor(alg) && neighbor_update);

    state->neighbor_change_send_hello |= send_hello;
    state->neighbor_change_send_hack |= send_hack;
...
void DF_uponHelloMessage(discovery_framework_state* state, HelloMessage* hello, WLANAddr* mac_addr) {
    ...
    if( neigh ) {
        ...
        if( NE_isDeleted(neigh) || rebooted ) {
            flushNeighbor(state, neigh);
            neigh = NULL;
    ...
    if( neigh ) {
        ...
        lost_messages = (seq_cmp - 1 - prev_missed_hellos);
        assert(lost_messages >= 0);

        state->stats.missed_hellos += lost_messages;

        NE_setNeighborSEQ(neigh, hello->seq);

        period_changed = NE_getNeighborHelloPeriod(neigh) != hello->period;
    ...
    // New Neighbor
        ...
        is_new_neigh = rebooted ? false : true;
        lost_messages = 0;
    ...
    // Update Link Quality
    double old_rx_lq = NE_getRxLinkQuality(neigh);
    double new_rx_lq = DA_computeLinkQuality(state->args->algorithm, NE_getLinkQualityAttributes(neigh), old_rx_lq, 1, lost_messages, is_new_neigh, &state->current_time);
    NE_setRxLinkQuality(neigh, new_rx_lq);

    bool updated_quality = false, updated_quality_threshold = false;
    double lq_delta = fabs(old_rx_lq - new_rx_lq);
    if( lq_delta > 0.0 ) {
        updated_quality = true;

        if( lq_delta >= state->args->lq_threshold ) {
            updated_quality_threshold = true;
        }
    }

    // Update traffic
    bool updated_traffic = false, updated_traffic_threshold = false;
    double traffic_delta = fabs(NE_getOutTraffic(neigh) - hello->traffic);
    if( traffic_delta > 0.0 ) {
        updated_traffic = true;

        if( lq_delta >= state->args->lq_threshold ) {
            updated_traffic_threshold = true;
        }
    }
    ...
    if( is_new_neigh ) {
        ...
        scheduleNeighborChange(state, true, false, false, false);

        insertIntoWindow(NT_getInstability(state->neighbors), &state->current_time, 1.0);
    } else if( updated_quality || updated_traffic || rebooted ) {
        DF_notifyUpdateNeighbor(state, neigh);

        if( updated_quality_threshold || updated_traffic_threshold ) {
            scheduleNeighborChange(state, false, false, true, false);
        }
    ...
    if( neigh && !NE_isDeleted(neigh) ) {
        ...
        if( uuid_compare(state->myID, hack->dest_process_id) == 0 ) {
            ...
            if( hack->neigh_type != LOST_NEIGH ) {
                ...
                state->stats.missed_hacks += (seq_cmp - 1 - prev_missed_hacks);

                bool new_hack = seq_cmp > 0;
                bool repeated_yet_fresh_hack = seq_cmp == 0 && hack->seq == state->my_seq-1;
                if( new_hack || repeated_yet_fresh_hack ) {
                    NE_setNeighborHSEQ(neigh, hack->seq);

                    // Check if the neighbor became symmetric (or bi)
                    bool became_bi = NE_getNeighborType(neigh, &state->current_time) != BI_NEIGH;
                    if( became_bi ) {
                    ...
                    milli_to_timespec(&tx_exp_time, hack->period*1000*state->args->hack_misses);
                    add_timespec(&tx_exp_time, &tx_exp_time, &state->current_time); */
            ...
            // The hack is meant for other node
        }

        // Update 2-hop neighborhood (Neighbor's neighbors)
        // If is a positive HACK
        if( hack->neigh_type != LOST_NEIGH ) {
            bool new_or_updated = addNeighborNeigh(neigh, hack->dest_process_id, hack->seq, hack->neigh_type == BI_NEIGH, hack->rx_lq, hack->tx_lq, hack->traffic, &hack_exp_time);
            if( new_or_updated ) {
                updated_neighbor = true;
            }
        }
        ...
        // If is a negative HACK
            ...
            bool removed = removeNeighborNeigh(neigh, hack->dest_process_id);
            assert(removed);
            updated_neighbor = true;
        }
...
void DF_piggybackDiscovery(discovery_framework_state* state, YggMessage* msg) {
    ...
    DA_createDiscoveryMessage(state->args->algorithm, state->myID, &state->current_time, state->neighbors, true, hello, hacks, n_hacks, buffer + sizeof(piggyback_size), &piggyback_size);

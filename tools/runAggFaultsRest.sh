...
for i in $(seq 1 $n_runs); do
  ...
  start_experience "$MULTITESTV2 $lowest_tree"
  ...
  STARTTIME=$(date +%s)
    ...
    stop_experience "${SRDS18}faults_${changes}_aggMultiTestV22_${lowest_tree}_$i $pis"
    echo "Killed Pis: $pis"
  ...
  stop_experience "${SRDS18}faults_${changes}_aggMultiTestV22_${lowest_tree}_$i"

  echo ""
  echo "---- MULTI V2 $i ended waiting $WAIT_TIME seconds.. -----"
  echo "---------lowest tree = $lowest_tree ----------"
  echo ""

...
if (($start_on < 2)); then
  ...
  for i in $(seq 1 $n_runs); do
    ...
    stop_experience "${SRDS18}remote_nofaults_aggMultiTest_${discov_period}_${msg_per_fault}_$i"

    echo ""
    echo "---- MULTI $i ended waiting $WAIT_TIME_TO_RUN seconds.. -----"
    echo ""
    ...
    stop_experience "${SRDS18}remote_nofaults_aggFlowTest_${discov_period}_${msg_per_fault}_$i"

    echo ""
    echo "----- FLOW $i ended waiting $WAIT_TIME_TO_RUN seconds.. -----"
    echo ""
    ...
    stop_experience "${SRDS18}remote_nofaults_aggGapTest_root_${gap_root}_${discov_period}_${msg_per_fault}_$i"

    echo ""
    echo "----- GAP $i ended waiting $WAIT_TIME_TO_RUN seconds.. ------"
    echo ""
    ...
    stop_experience "${SRDS18}remote_nofaults_aggGapBcastTest_root_${gap_root}_${discov_period}_${msg_per_fault}_$i"

    echo ""
    echo "--- GAPBCAST $i ended waiting $WAIT_TIME_TO_RUN seconds.. ---"
    echo ""
    sleep $WAIT_TIME_TO_RUN
  done
...
stop_experience "${SRDS18}remote_nofaults_aggLimoTest_${discov_period}_${msg_per_fault}_1"

echo ""
echo "----- LIMO 1 ended waiting $WAIT_TIME_TO_RUN seconds.. -----"
echo ""

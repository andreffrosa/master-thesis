...
case "$1" in
  ...
  configfile=$YGGHOME"/runonboot.txt"
  ...
  if [ -f $configfile ]; then
    ...
    if [ ! -z "$command" ]; then
       c=$(ls -1 /var/log/lightkone/errors.log.* | wc -l)
       c=$((c+1))
  ...
  if [ -f /home/yggdrasil/output.out ]; then
     count=$(ls -1 /var/log/lightkone/output.out* | wc -l)
     count=$((count+1))
     mv /home/yggdrasil/output.out /var/log/lightkone/output.out.$count
  fi
	;;
  status)
	;;

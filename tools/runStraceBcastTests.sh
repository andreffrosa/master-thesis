...
DURATION="40" #s duration of exp
COOL_PERIOD="5" #s time to wait before kill while not sending new messages
...
for (( i=0; i < $n_exp; i++ )); do
  ...
  start_experience "strace -o $STRACE_DIR${EXP_NAME[i]}_run$r.log ${EXPERIMENTS[i]}"
